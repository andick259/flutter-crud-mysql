...
class Jockey extends StatefulWidget {
  const Jockey({Key? key}) : super(key: key);

  @override
  State<Jockey> createState() => _JockeyState();
}

class _JockeyState extends State<Jockey> {
  ...
  Future<void> getProductData() async {
    ...
    try {
      final response = await http.post(
          Uri.parse('https://mitrasite.com/nearco/api/tampil-jockey.php'));

      if (response.statusCode == 200) {
        setState(() {
          jockeyList = jsonDecode(response.body);
          isLoading = false;
        });
      } else {
        throw Exception('Failed to load Jockey');
      }
    } catch (e) {
      setState(() {
        isLoading = false;
      });
      print('Error fetching jockey: $e');
    }
  }
  ...
  void navigateToDetailCoach(String idJockey) {
    //Navigator.push(
    // context,
    // MaterialPageRoute(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                else if (jockeyList.isEmpty)
                  Center(
                    child: Text('No jockey found'),
                  )
                else
                  // Product grid view
                  GridView.builder(
                    ...
                    itemCount: jockeyList.length,
                    itemBuilder: (BuildContext context, int index) {
                      var jockey = jockeyList[index];
                        ...
                        child: Container(
                          decoration: BoxDecoration(
                            ...
                            image: DecorationImage(
                              image: NetworkImage(
                                  'https://mitrasite.com/nearco/app/foto_jockey/${jockey['foto']}'),
                              fit: BoxFit.cover,
                            ),
                          ),
                          child: Column(
